---
title: "Assignm ent Bioinformatics Basics Report"
author: "Jining Zhong"
date: "02/18/2026"
...
library(ggplot2)
gene_order <- long_format_data %>%
  dplyr::distinct(hgnc_symbol, gene_set) %>%
  dplyr::arrange(desc(gene_set)) %>%  
  dplyr::pull(hgnc_symbol)

ggplot(long_format_data, aes(x = factor(hgnc_symbol, levels = gene_order), y = value, fill = gene_set)) +
  geom_boxplot() +
  scale_fill_manual(values = c("bad" = "#E07B6A", "good" = "#6BBDBC")) +
  labs(
